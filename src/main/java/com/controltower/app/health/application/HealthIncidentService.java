...
public class HealthIncidentService {
    ...
    public void evaluateAfterCheck(ClientBranch branch, HealthCheck latestCheck) {
        ...
        if (rules.isEmpty()) {
            ...
            if (latestCheck.getStatus() == HealthCheck.HealthStatus.DOWN) {
                ...
                long previousConsecutiveDown = recent.stream()
                        .limit(2)
                        .filter(c -> c.getStatus() == HealthCheck.HealthStatus.DOWN
                                || c.getStatus() == HealthCheck.HealthStatus.UNKNOWN)
                        .count();
                long consecutiveDown = 1 + previousConsecutiveDown;
        ...
        if (shouldOpenIncident && openIncident.isEmpty()) {
            ...
            eventPublisher.publishEvent(new HealthIncidentResolvedEvent(incident, null));
    ...
    public void resolve(java.util.UUID incidentId, java.util.UUID resolvedBy, String resolutionNote) {
        ...
        incidentRepository.save(incident);
        eventPublisher.publishEvent(new HealthIncidentResolvedEvent(incident, resolvedBy));
    }
}
